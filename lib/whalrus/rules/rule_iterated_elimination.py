...
class RuleIteratedElimination(Rule):
    ...
    args
        Cf. parent class.
    base_rule : Rule
        The rule used at each round to determine the eliminated candidate(s). Unlike for
        :class:`RuleSequentialElimination`, all the rounds use the same voting rule.
    elimination : Elimination
        The elimination algorithm. Default: ``EliminationLast(k=1)``.
    propagate_tie_break : bool
        If True (default), then the tie-breaking rule of this object is also used for the base rule (cf. below).
    kwargs
        Cf. parent class.
    ...
    def eliminations_(self) -> list:
        """list: The elimination rounds. A list of :class:`Elimination` objects. The first one corresponds to the first
        round, etc.
        """
