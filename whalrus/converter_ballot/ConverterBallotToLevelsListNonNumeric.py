...
class ConverterBallotToLevelsListNonNumeric(ConverterBallot):
    ...
    def __call__(self, x: object, candidates: set =None) -> BallotLevels:
        x = ConverterBallotToLevelsInterval(
            low=0., high=1., borda_unordered_give_points=self.borda_unordered_give_points
        )(x, candidates=None)
        return BallotLevels({c: self.scale.levels[round(v * (len(self.scale.levels) - 1))] for c, v in x.items()},
