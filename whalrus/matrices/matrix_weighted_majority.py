...
class MatrixWeightedMajority(Matrix):
    ...
    higher_vs_lower : Number or None
        Number of points for candidate `c` when it is ordered higher than candidate `d`.
    lower_vs_higher : Number or None
        Number of points for candidate `c` when it is ordered lower than candidate `d`.
    indifference : Number or None
        Number of points for candidate `c` when it is ordered and tied with candidate `d`.
    ordered_vs_unordered : Number or None
        Number of points for candidate `c` when it is ordered and `d` is unordered.
    unordered_vs_ordered : Number or None
        Number of points for candidate `c` when it is unordered and `d` is ordered.
    unordered_vs_unordered : Number or None
        Number of points for candidate `c` when it is unordered and `d` is unordered.
    ordered_vs_absent : Number or None
        Number of points for candidate `c` when it is ordered and `d` is absent.
    absent_vs_ordered : Number or None
        Number of points for candidate `c` when it is absent and `d` is ordered.
    unordered_vs_absent : Number or None
        Number of points for candidate `c` when it is unordered and `d` is absent.
    absent_vs_unordered : Number or None
        Number of points for candidate `c` when it is absent and `d` is unordered.
    absent_vs_absent : Number or None
        Number of points for candidate `c` when it is absent and `d` is absent.
    diagonal_score : Number
        Value of the diagonal coefficients.
    default_score : Number
        Default score in the matrix in case of division by 0 (except for the diagonal coefficients).
    antisymmetric : bool
        If True, then an antisymmetric version of the matrix is computed (by subtracting the transposed matrix at the
    ...
    @cached_property
    def gross_(self):
        """NiceDict: The "gross" matrix. Keys are pairs of candidates. Each coefficient is the weighted number of
        points (used as numerator in the average).
    ...
    @cached_property
    def weights_(self):
        """NiceDict: The matrix of weights. Keys are pairs of candidates. Each coefficient is the total weight (used as
