...
def test_two_elements_or_more():
    ...
    with pytest.raises(ValueError):
        _ = priority.choose(my_set)
    with pytest.raises(ValueError):
        _ = priority.choose(my_set, reverse=True)
    with pytest.raises(ValueError):
    ...
    assert priority.choose(my_set) is None
    assert priority.choose(my_set, reverse=True) is None
    assert priority.sort(my_set) is None
    assert priority.sort(my_set, reverse=True) is None

    priority = Priority.ASCENDING
    assert priority.choose(my_set) == 'a'
    assert priority.choose(my_set, reverse=True) == 'd'
    assert priority.sort(my_set) == ['a', 'b', 'c', 'd']
    assert priority.sort(my_set, reverse=True) == ['d', 'c', 'b', 'a']

    priority = Priority.DESCENDING
    assert priority.choose(my_set) == 'd'
    assert priority.choose(my_set, reverse=True) == 'a'
    assert priority.sort(my_set) == ['d', 'c', 'b', 'a']
    assert priority.sort(my_set, reverse=True) == ['a', 'b', 'c', 'd']

    priority = Priority.RANDOM
    assert priority.choose(my_set) in my_set
    assert priority.choose(my_set, reverse=True) in my_set
